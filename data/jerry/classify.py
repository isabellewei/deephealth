...
cols = list(df)
f1 = [x for x in cols if x != 'admission_type_id']
f2 = [x for x in cols if x != 'discharge_disposition_id']
features1 = df[list(f1)].values
features2 = df[list(f2)].values
f1_train, f1_test, l1_train, l1_test = train_test_split(features1, labels1, test_size=0.25, random_state=42)
f2_train, f2_test, l2_train, l2_test = train_test_split(features2, labels2, test_size=0.25, random_state=42)
